...
xively.controller('selectController', ['$scope','$rootScope','Socket','localStorageService','sharedProperties','VisitorsService','storeService','$http','$location','SubscriptionFactory', 'LSFactory', 'API_URL','$window','$timeout','ngToast','OrdersService',function($scope,$rootScope, Socket,localStorageService,sharedProperties,VisitorsService,storeService,$http,$location,SubscriptionFactory, LSFactory, API_URL,$window,$timeout,ngToast,OrdersService){
    ...
    // Start SideMenu
        ...
        $scope.order=function(){
			...
			$scope.cleanOrders = OrdersService.getOrders();
			 for(var k in $scope.cleanOrders) {
                 console.log(k, $scope.cleanOrders[k]);
            }
			*/
